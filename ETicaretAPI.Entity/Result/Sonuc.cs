...
public class Sonuc<T>
{
    ...
    public static Sonuc<T> SuccessDataNotFound(string message="Sonuç Bulunamadı",int statusCode = (int)HttpStatusCode.NotFound)
    {
        return new Sonuc<T>(message, statusCode,HataBilgisi.NotFound());
    }

    public static Sonuc<T> AuthenticationError(string message = "Sonuç Bulunamadı",int statusCode = (int)HttpStatusCode.NotFound)
    {
